...
extension BarChartDataExtension on BarHChartData {
  List<double> calculateGroupsX(double viewWidth) {
    ...
    switch (alignment) {
      ...
      case BarHChartAlignment.center:
        var sumWidth =
      ...
      case BarHChartAlignment.spaceBetween:
        final sumWidth =
            barGroups.map((group) => group.width).reduce((a, b) => a + b);
      ...
      case BarHChartAlignment.spaceAround:
        final sumWidth =
            barGroups.map((group) => group.width).reduce((a, b) => a + b);
        final spaceAvailable = viewWidth - sumWidth;
      ...
      case BarHChartAlignment.spaceEvenly:
        final sumWidth =
            barGroups.map((group) => group.width).reduce((a, b) => a + b);
        final spaceAvailable = viewWidth - sumWidth;
        final eachSpace = spaceAvailable / (barGroups.length + 1);
